...
echo "[COMMAND] removing directories"
...
echo "[COMMAND] creating directoryes"
...
:'
dir="$cwllib_dir/s1docker_dir1"
old_image=$doc_image1
new_image="viki_image1"
echo "[EXECUTING] docker run"
docker run -i $old_image mkdir -p $dir  
echo "[GETTING CONTAINER ID]"
con_id=`docker container ls -l --format {{.ID}}" "{{.Image}} | grep $old_image | awk '{print \$1}'`
echo "[CONTAINER ID] [$con_id]"
echo "[COMMITTING and CREATING] viki_image1"
docker commit -a "Viki" -m "test" $con_id $new_image
echo "[VERIFYING IMAGE] viki_image1"
...
echo "[EXECUTING] docker run"
dir="$cwllib_dir/s1docker_dir2"
old_image=$doc_image2
new_image="viki_image2"
docker run -i $old_image mkdir -p $dir  
echo "[GETTING CONTAINER ID]"
con_id=`docker container ls -l --format {{.ID}}" "{{.Image}} | grep $old_image | awk '{print \$1}'`
echo "[CONTAINER ID] [$con_id]"
...
#echo "[EXECUTING] docker run"
dir="$cwllib_dir/s1docker_dir3"
old_image=$doc_image3
new_image="viki_image3"
docker run -i $old_image mkdir -p $dir  
docker run -i $old_image mkdir -p "$cwllib_dir/s1docker_dir4"
echo "[GETTING CONTAINER ID]"
con_id=`docker container ls -l --format {{.ID}}" "{{.Image}} | grep $old_image | awk '{print \$1}'`
echo "[CONTAINER ID] [$con_id]"
echo "[COMMITTING and CREATING] viki_image1"
...
doc_image1="viki_image1"
doc_image2="viki_image2"
doc_image3="viki_image3"
doc_image4="viki_image3"
'
...
docker run -i \
 ...
 --env=HOME=/var/spool/cwl \
 $doc_image2 pigz -c \
 s2l1=$cwllib_dir/stge258482e-6be1-4a52-bf63-815c1396e154/megahit.log 
...
--env=TMPDIR=/tmp \
--env=HOME=/var/spool/cwl \
$doc_image3 bbmap.sh -Xmx2000M nodisk=t out=stdout.sam \
in=$s3gz1 \
in2=$s3gz2 \
ref=$s3fa1 
...
--env=TMPDIR=/tmp \
--env=HOME=/var/spool/cwl \
$doc_image4 bbmap.sh -Xmx2000M nodisk=t out=stdout.sam \
in=$s4gz1 \
in2=$s4gz2 \
ref=$s4fa1 
